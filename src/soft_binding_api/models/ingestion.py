...
``IngestionRecord`` is the persisted shape (Mongo + JSON sidecar).
...
class IngestionRecord(BaseModel):
    """
    Persisted ingestion metadata. Stored both in Mongo (``ingestions``
    collection) and as a JSON sidecar next to the watermarked output on the
    local filesystem.
    """
