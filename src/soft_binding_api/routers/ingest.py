...
watermark + manifest + sign pipeline, and returns JSON describing the
resulting artifacts (with download URLs). Two helpers expose the bytes:

- ``GET /ingest/{ingestionId}/asset`` returns the signed audio.
- ``GET /ingest/{ingestionId}/manifest`` returns the raw signed manifest
  bytes (when the SDK surfaced them).
- ``GET /ingest/{ingestionId}`` returns the persisted metadata.
"""
...
from soft_binding_api.models.ingestion import IngestionRecord, IngestResponse
from soft_binding_api.services.ingestion import (
    IngestionInput,
    IngestionService,
)
from soft_binding_api.services.ingestion.service import (
    IngestionError,
...
def get_repo(request: Request) -> IngestionRepository:
    repo: IngestionRepository | None = getattr(request.app.state, "ingestion_repo", None)
    if repo is None:
        raise HTTPException(status_code=503, detail="Ingestion repository not initialised")
    return repo
...
@router.post(
    ...
    summary="Ingest an audio asset: watermark, build C2PA manifest, sign, store",
    responses={
        ...
        503: {"description": "Service not ready (Mongo down or certs missing)"},
    },
)
async def ingest_audio(
    ...
    try:
        ...
        # Missing certs typically end up here; surface a clean 503.
...
async def get_ingestion(
    ...
    record = await repo.get(ingestionId)
    if record is None:
...
async def get_signed_asset(
    ...
    record = await repo.get(ingestionId) or store.load_metadata(ingestionId)
