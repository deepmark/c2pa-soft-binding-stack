...
Pipeline failures (after we've allocated an ingestion_id) get persisted
to the ``failed_ingestions`` collection with whatever metadata was
knowable at the failure point. Pre-allocate failures (unsupported MIME,
bad alg request) raise as 4xx and are not persisted.
...
class IngestionService:
    ...
    async def ingest(self, payload: IngestionRequest) -> IngestionOutput:
        ...
        # any failure gets persisted to ``failed_ingestions``.
