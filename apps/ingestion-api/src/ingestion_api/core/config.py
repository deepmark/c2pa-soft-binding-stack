...
- writes an ``IngestionRecord`` to the ``ingestions`` collection,
...
Required env vars: ``MONGODB_URL``, ``STORAGE_ROOT``, ``CREDENTIALS_DIR``.
...
class Settings(BaseSettings):
    ...
    mongodb_database: str = Field(
        ...
        description="Mongo database containing ingestion and plugin catalog collections.",
    )
    ingestions_collection: str = "ingestions"
    ...
    # Persistent artifact store. Metadata is stored in MongoDB.
    ...
    # Fine when the pod is only reachable from a known proxy.
