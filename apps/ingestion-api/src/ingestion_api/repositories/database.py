...
The ``ingestions`` collection stores every ingestion lifecycle state:
...
class MongoDB:
    ...
    async def _ensure_indexes(cls) -> None:
        ...
        await cls._ensure_ingestions_indexes(cls.db[settings.ingestions_collection])
    ...
    async def _ensure_ingestions_indexes(col: AsyncIOMotorCollection) -> None:
        ...
        await col.create_index(
            ...
            name="status_created_at_id_desc_idx",
        )
...
def get_ingestions_collection() -> AsyncIOMotorCollection:
    ...
    return MongoDB.db[settings.ingestions_collection]
