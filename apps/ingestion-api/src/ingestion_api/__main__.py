...
A missing or malformed cert fails lifespan startup rather than throwing 503s
on the first ingest.
"""
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Connected to MongoDB at %s db=%s", settings.mongodb_url, settings.mongodb_database)
    ...
    if not plugin_catalog:
        logger.warning(
            "No algorithms found in %s.%s at startup. "
            "Algorithms will be resolved from MongoDB on each request.",
            settings.mongodb_database,
            settings.supported_algorithms_collection,
        )
    else:
        logger.info(
            "Found %d plugin entries in %s.%s: %s",
            len(plugin_catalog),
            settings.mongodb_database,
            settings.supported_algorithms_collection,
