...
from ingestion_api.repositories.ingestions import InMemoryIngestionRecordRepository
...
def test_ingest_produces_signed_asset_and_record(
    ...
    assert record.status is IngestionStatus.SUCCEEDED
...
def test_ingest_records_failed_push(
    ...
    The ingest lifecycle still succeeded; only the downstream push failed."""
    ...
    assert result.record.status is IngestionStatus.SUCCEEDED
...
def test_pipeline_failure_persists_failed_ingestion(
    ...
    """Plugin /embed unreachable -> IngestionError + failed IngestionRecord."""
    entry = PluginEntry(
        alg=BINDING_ALG,
        type="watermark",
        binding_bits=128,
    ...
    failures = list(records._records.values())  # noqa: SLF001
    ...
    assert all(
        record.status is not IngestionStatus.SUCCEEDED
        for record in records._records.values()  # noqa: SLF001
    )
...
def test_pipeline_rejects_plugin_that_changes_audio_format(
    ...
    failures = list(records._records.values())  # noqa: SLF001
    assert len(failures) == 1
    assert failures[0].status is IngestionStatus.FAILED
...
def test_ingest_rejects_unknown_alg(
    ...
    # Pre-allocate failure -> nothing persisted.
